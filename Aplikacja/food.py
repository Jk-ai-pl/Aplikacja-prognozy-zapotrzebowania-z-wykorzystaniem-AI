...
class Food:
    def __init__(self, master):
        ...
        self.window.title("Zasoby żywnościowe")
    ...
    def generate_forecast(self):
        ...
        self._create_forecast_window(dates, demand, "Prognoza zapotrzebowania na zasoby żywnościowe",
                                     f"Żywnościowe ({unit})", unit)
    ...
    def _create_forecast_window(self, dates, demand, title, label, unit):
        ...
        slider = Scale(forecast_win, from_=0, to=max(0, len(dates)-12), orient=HORIZONTAL,
                       command=update_xlim, showvalue=0)
        slider.pack(fill="x")
        update_xlim(0)
