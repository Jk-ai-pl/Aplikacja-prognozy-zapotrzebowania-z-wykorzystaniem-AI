...
class Construction:
    def __init__(self, master):
        ...
        self.window.title("Zasoby budowlane")
    ...
    def generate_forecast(self):
        ...
        forecast_win.title("Prognoza zapotrzebowania")
        forecast_win.geometry("1000x600")

        fig, ax = plt.subplots(figsize=(len(dates)/2, 5))
        line, = ax.plot(dates, demand, marker='o', label=f"Budowlane ({unit})")
        ax.set_xlabel("Data")
        ax.set_ylabel(f"Ilość zasobów ({unit})")
        ax.set_title("Prognoza zapotrzebowania na zasoby budowlane")
        ax.grid(True)
